	...
	tests := []test{
		{
			...
			cpMock: &th.CredsProviderMock{
				CreateTokenFunc: func(input credentials.CreateTokenInput) (credentials.CreateTokenOutput, error) {
					return credentials.CreateTokenOutput{
						Token: types.Token{
	...
	tests := []test{
		{
			...
			url:        "/workflows",
			cpMock: &th.CredsProviderMock{
				GetTokenFunc: func(input credentials.GetTokenInput) (credentials.GetTokenOutput, error) {
...
// TODO double check these; the approach is different now as we're using a
// token exists call.
func TestDeleteToken(t *testing.T) {
	tests := []test{
		...
		},
		{
			...
			method:     "DELETE",
			cpMock: &th.CredsProviderMock{
				DeleteProjectTokenFunc: func(input credentials.DeleteProjectTokenInput) (credentials.DeleteProjectTokenOutput, error) {
					return credentials.DeleteProjectTokenOutput{}, nil
				},
				ProjectExistsFunc: projExistsFunc(true),
				ProjectTokenExistsFunc: func(input credentials.ProjectTokenExistsInput) (credentials.ProjectTokenExistsOutput, error) {
					return credentials.ProjectTokenExistsOutput{Exists: true}, nil
				},
			},
			dbMock: &th.DBClientMock{
		...
		// },
		{
			...
			cpMock: &th.CredsProviderMock{
				ProjectExistsFunc: projExistsFunc(true),
				ProjectTokenExistsFunc: func(input credentials.ProjectTokenExistsInput) (credentials.ProjectTokenExistsOutput, error) {
					return credentials.ProjectTokenExistsOutput{Exists: false}, nil
				},
			},
			dbMock: &th.DBClientMock{
				ReadProjectEntryFunc: func(ctx context.Context, project string) (db.ProjectEntry, error) {
		...
		},
		{
			...
			cpMock: &th.CredsProviderMock{
				DeleteProjectTokenFunc: func(input credentials.DeleteProjectTokenInput) (credentials.DeleteProjectTokenOutput, error) {
					return credentials.DeleteProjectTokenOutput{}, errors.New("error deleting token from Vault")
				},
				ProjectExistsFunc: projExistsFunc(true),
				ProjectTokenExistsFunc: func(input credentials.ProjectTokenExistsInput) (credentials.ProjectTokenExistsOutput, error) {
					return credentials.ProjectTokenExistsOutput{Exists: true}, nil
